...
public class ScenarioGenerator {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 1; i <= 10; i++) {
            ...
            // Define profile based on ID
            // 1-4: Readers (mostly find)
            // 5-7: Writers (save/update/delete)
            // 8-10: Luxury (high prices)
            ...
            System.out.println(((("Simulating " + userId) + " [") + profile) + "]");
    ...
    private static void simulateUser(String userId, String profile) {
        ...
        for (int j = 0; j < 20; j++) {
            try {
                ...
                // Ignore exceptions for simulation flow, just log internally if needed
            }
            // Small pause to separate timestamps slightly
            try {
                Thread.sleep(10);
    ...
    private static void doRead() {
        // Mostly findAll or findById
        if (random.nextBoolean()) {
            repository.findAll();
        } else {
            // Random ID lookup - might fail but produces log
            try {
        ...
        switch (action) {
            case 0 :
                try {
                    ...
                    // If exists, define as update intent for simulation
                    repository.update(p);
                }
                break;
            case 1 :
